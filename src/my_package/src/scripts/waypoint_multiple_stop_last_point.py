...
class WaypointFollower:
    def __init__(self):
        ...
        self.pose = Pose()
        self.target_poses = [
            (1.0, 1.0),   # Waypoint 1: (x, y) = (1.0, 1.0)
            (2.0, 2.0),   # Waypoint 2: (x, y) = (2.0, 2.0)
            (3.0, 3.0)   # Waypoint 3: (x, y) = (3.0, 3.0)
            
        ]
        self.current_waypoint = 0
        self.stay_durations = [1, 2, 3]  # Stay durations at each waypoint (in seconds)
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.reached_waypoint():
                ...
                if self.current_waypoint == len(self.target_poses) - 1:
                    rospy.loginfo("Final waypoint reached. Stopping the turtle.")
                    break  # Exit the loop
